...
function activate(context) {
    ...
    // コマンドの登録
    context.subscriptions.push(vscode.commands.registerCommand('manageNpmPkg.refresh', () => npmProvider.refresh()), 
    ...
    vscode.commands.registerCommand('manageNpmPkg.add', async () => {
        ...
        const data = (await res.json()); // (型定義があれば NpmSearchResponse に置き換え)
        ...
        const installType = await vscode.window.showQuickPick([{ label: 'Dependencies' }, { label: 'Dev Dependencies' }], { placeHolder: `${selectedPkg.label} のインストール先を選択してください` });
        if (!installType) {
            return;
        }
        const isDev = installType.label === 'Dev Dependencies';
        // 6. 確認ダイアログを表示
        const answer = await vscode.window.showInformationMessage(`パッケージ '${selectedPkg.label}' を「${isDev ? '開発' : '通常'}依存関係」としてインストールしますか？`, { modal: false }, 'インストールする');
        // 7. ターミナルで実行
        if (answer === 'インストールする') {
            runTerminalCommand(isDev ? `pnpm add -D ${selectedPkg.label}` : `pnpm add ${selectedPkg.label}`);
        }
    }), vscode.commands.registerCommand('manageNpmPkg.openInfo', (item) => {
    ...
    // 削除時の確認ダイアログ
    vscode.commands.registerCommand('manageNpmPkg.remove', async (item) => {
        // showWarningMessage の第2引数に { modal: true } を渡すと、画面中央にダイアログが出ます
        const answer = await vscode.window.showWarningMessage(`パッケージ '${item.label}' を本当にアンインストールしますか？`, { modal: true }, 'アンインストールする');
